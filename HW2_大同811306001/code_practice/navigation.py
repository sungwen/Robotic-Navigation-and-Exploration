    ...
    while(True):
        ...
        if path is not None and collision_count == 0:
            ...
            info = {
                ...
                "v": simulator.state.v,
                "delta": simulator.state.delta if args.simulator == "bicycle" else 0,
                "l": simulator.l,
                "dt": simulator.dt
            }
            ...
            if args.simulator == "basic":
                ...
                next_v = 5.0  # Set constant speed for basic model
                command = ControlState("basic", next_v, next_w)

            elif args.simulator == "diff_drive":
                next_w, _ = controller.feedback(info)
                v = 5.0  # Set constant forward speed
                r = simulator.wu / 2
                l = simulator.l
                next_lw = (v - next_w * l) / r
                next_rw = (v + next_w * l) / r
                command = ControlState("diff_drive", next_lw, next_rw)

            elif args.simulator == "bicycle":
                next_delta, _ = controller.feedback(info)
                next_a = 1.0 if simulator.state.v < 10 else 0.0  # Simple acceleration control
        ...
        if collision_count > 0:
            ...
            for _ in range(10):
                simulator.step(ControlState(args.simulator, -2, 0))  # Move backward slowly
            collision_count = 0  # reset collision_count after moving back
            set_controller_path = True  # Trigger re-planning
